...
void guardar (ESTADO e, char nome[]) { // nome é sem extensão, nome2 é COM extensão
    ...
    if (e.modo == '0')
        strcpy (conteudo, "M ");  // !!!! ACRESCENTEI ESPAÇO !!!!
    else
        strcpy (conteudo, "A ");  // !!!! ACRESCENTEI ESPAÇO !!!!
...
int verificaLinha (ESTADO e, int dl, int dc, int linha, int coluna) {
    ...
    if(e.grelha[linha][coluna] == VAZIA) return 1;         // !!!! ACRESCENTEI CONDIÇÃO !!!!
 ...
 * Retorna o valor 1 se tiver de passar a jogada.
 * Caso contrário, retorna o valor 0.
 */
...
ESTADO jogar (ESTADO e, int linha, int coluna) {
    ...
    if(passaJogada(e, e.peca)==1){
        ...
        if (passaJogada(e, outro)==1) e = fimJogo(e);
            ...
            printf("\nJogadas indisponíveis. Passou jogada.\n");
            e.peca = outro;
